...
import {
  PATIENT_ACTIONS,
  setCreatePatientFailure,
  setCreatePatientSucceed,
  setDeletePatientFailure,
  setDeletePatientSucceed,
  setFetchPatientFailure,
  setFetchPatientSucceed,
  setUpdatePatientFailure,
  setUpdatePatientSucceed,
...
function* createPatient(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setCreatePatientFailure(`[create Patient] : ${error.toString()}`)
      );
      throw error;
    }
...
function* updatePatient(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setUpdatePatientFailure(`[update Patient] : ${error.toString()}`)
      );
      throw error;
    }
...
function* deletePatient(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setDeletePatientFailure(`[delete Patient] : ${error.toString()}`)
      );
      throw error;
    }
